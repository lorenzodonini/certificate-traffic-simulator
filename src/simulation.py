...
class Simulation:
    ...
    def __instantiate_node__(self, space):
        ...
        space.add_node(n)
        if self.config.mode == SimulationMode.INCREMENTAL_SERVICE_SPAWN \
                or self.config.mode == SimulationMode.INCREMENTAL_MIXED_SPAWN:
            return
    ...
    def __log_data__(self, space, traffic):
        if self.logger:
            ...
            self.logger.log_data(self.network_manager.current_time,
                                 ...
                                 services,
                                 traffic,
                                 (self.network_manager.monitored_traffic / self.network_manager.current_time))
    ...
    def run(self):
        ...
        while self.__tick__():
            if self.config.mode == SimulationMode.INCREMENTAL_NODE_SPAWN \
                    or self.config.mode == SimulationMode.INCREMENTAL_MIXED_SPAWN:
                self.__instantiate_node__(self.space)
            ...
            traffic = self.network_manager.monitored_traffic - traffic
            self.__log_data__(self.space, traffic)
...
def parse_args():
    ...
    parser.add_argument('-m', '--mode', help="Mode (0,1). Mode 0 instantiates 1 more node every minute. Mode 1 instantiates all nodes at startup", type=SimulationMode, default=SimulationMode.INCREMENTAL_SPAWN, dest='mode')
