...
class DataManager:
    def __init__(self, output_path):
        ...
        fieldnames = ['sim_time', 'nodes', 'services', 'generated_traffic_B', 'average_traffic_Bps', 'cert_validity']
        self.writer = csv.DictWriter(self.out_file, fieldnames=fieldnames)
        self.writer.writeheader()

    def log_data(self, time, nodes, services_per_node, generated_traffic, traffic_per_second, cert_validity):
        row = dict()
        row['sim_time'] = time
        row['nodes'] = nodes
        ...
        row['cert_validity'] = cert_validity
